...
'''
Main function for parsing a NorthWestern dataset file. This file's structure is
a json file that consists of all the data tables as a json oject.
'''          
def main():
    inp = gzip.open("tables.json.gz", 'rt', encoding="UTF-8")
    out = open("NorthWesternTables.txt", "w+", encoding="UTF-8")
    ...
    for line in inp:
        ...
        num_rows = int(data["numDataRows"])
        num_cols = int(data["numCols"])
        columns = []
        for i in range(num_cols):
            columns.append([])    
        for i in range(num_rows):
            for j in range(num_cols):
                columns[j].append(data["tableData"][i][j]["text"])

        # Variable that will contain columns that are valid.
        # A column is valid if it is not empty, does not have an average
        # length longer than 40 characters, is not the same entry repeating
    ...
    print(table_count)  
    print(sum(row_size) / len(row_size))
    print(sum(col_size) / len(col_size))
    inp.close()
    out.close()
