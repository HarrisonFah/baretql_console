...
def main():
    ...
    inp = tarfile.open("01.tar")
    out = open("WDCTables.txt", "w+", encoding="UTF-8")
    ...
    for file in inp:
        if file.offset == 0:
            continue
        extracted_file = inp.extractfile(file)  
        ...
        num_rows = len(data["relation"][0])
        num_cols = len(data["relation"])
        ...
        for i in range(num_cols):
            columns.append(data["relation"][i])
...
def switch_key_col(cols):
    if len(cols) > 0:
        key_col = cols[0] 
        tag = UnitTagging.tag_unit_for_column(key_col)
        ...
        if ((tag == "numeric") or 
            ...
            for i in range(1, len(cols)):
                tag = UnitTagging.tag_unit_for_column(cols[i])
                if (tag == "text" and 
